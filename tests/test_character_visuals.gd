...
func run() -> Array[Dictionary]:
	...
	_test_outfit_disabled(results, visual_scene)
	_test_outfit_disable_is_idempotent(results, visual_scene)
...
func _test_outfit_disabled(results: Array[Dictionary], visual_scene: PackedScene) -> void:
	var catalog := load(CATALOG_RESOURCE_PATH) as HumanCharacterCatalog
	if catalog == null:
		return
	for role in [&"player", &"civilian", &"hostile"]:
		var visual := _create_visual(visual_scene)
		var configured := visual.configure_from_catalog(catalog, "outfit-seed-%s" % role, role, 1.70)
		_expect(results, "%s catalog visual keeps imported body clothing only" % role, configured and _has_no_outfit_geometry(visual))
		_dispose_visual(visual)


func _test_outfit_disable_is_idempotent(results: Array[Dictionary], visual_scene: PackedScene) -> void:
	var visual := _create_visual(visual_scene)
	var first_configured := visual.configure_body(MALE_BODY_PATH, 1.70)
	visual.call("_configure_outfit")
	visual.call("_configure_outfit")
	_expect(results, "direct body configuration disables outfit geometry", first_configured and _has_no_outfit_geometry(visual))
	visual.call("_clear_body")
	_expect(results, "body clearing leaves no stale outfit nodes or attachments", visual.get_body_root() == null and _has_no_outfit_geometry(visual))
	var second_configured := visual.configure_body(FEMALE_BODY_PATH, 1.82)
	_expect(results, "reconfiguration after clearing remains outfit-free", second_configured and _has_no_outfit_geometry(visual))
	_dispose_visual(visual)


func _has_no_outfit_geometry(visual: HumanCharacterVisual) -> bool:
	return not visual.has_outfit_torso() and not visual.has_outfit_detail() and visual.get_outfit_torso_node() == null and visual.get_outfit_detail_node() == null and visual.get_outfit_torso_attachment() == null and visual.get_outfit_detail_variant() == -1 and visual.get_outfit_detail_name().is_empty() and visual.get_node_or_null("ModelPivot/SharedLowPolyOutfit") == null
